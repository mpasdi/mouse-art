...
function specificDirectMove(deg?: number, distance: number = 100) {
  ...
  const translateX =
    deg || deg === 0 ? Math.cos((Math.PI / 180) * deg) * distance : randomOne * 100 * signType()
  const translateY =
    deg || deg === 0 ? Math.sin((Math.PI / 180) * deg) * distance : randomTwo * 100 * signType()
  ...
  const animation = [
    ...
    `
      .${unique_className} {
        animation: ${unique_className} var(--duration) ease-out;
